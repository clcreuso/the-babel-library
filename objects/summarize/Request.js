...
const getRequestIntroduction = (data) => {
  if (data.language === 'French')
    return [
      `Objectif : Créer une introduction clair en français, au format HTML`,
      `Conditions :`,
...
const getRequestChapter = (data) => {
  if (data.language === 'French')
    return [
      ...
      `Conditions :`,
      `- Lire et analyser l'intégralité du contenu fourni avant de commencer le résumé.`,
      ...
      `- Garder une forme d'écriture digeste.`,
      `Format attendu :`,
...
const getRequestConclusion = (data) => {
  if (data.language === 'French')
    return [
      `Objectif : Créer une conclusion clair en français, au format HTML`,
      `Conditions :`,
      `- Utilse uniquement ces balises: <h>, <p>, <b>, <i>, <br>`,
      `Réponse attendue (en JSON) :`,
      `{ "content": "HTML" }`,
...
const sendRequestContent = (data) => {
  if (data.type === 'introduction') {
    return getRequestIntroduction(data);
  }

  if (data.type === 'conclusion') {
    return getRequestConclusion(data);
  }
